...
class Ctrl(PX4MavCtrler):
    ...
    def send_full(self, pos, vel, acc, yaw_rate):
        if self.simulinkDLL:
            self.SendSynFull(pos[0], pos[1], pos[2], vel[0], vel[1], vel[2], acc[0], acc[1], acc[2], yaw_rate)
...
class PID:
    ...
    def pid(self, err, dt):
        if dt < 0.002:
            dt = 0.002
        rec = self.p * err
        self.sum_err = self.sum_err + err*dt
    ...
    def set_sum_limit(self, sum_limit):
        self.sum_limit = sum_limit

    def get_sum_err(self):
        return self.sum_err
