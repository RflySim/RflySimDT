...
import math
import threading
from src.control.utils.RflySoftwarm import REG
...
class Command:
    ...
    def Arm(self):  # ID = 1
        self.isDone = 0
        # print(f'Mav{self.ID}等待其他飞机同步解锁')
        REG.barrier.wait()
    ...
    def QuadTakeoff(self,pos): # ID = 3
        self.isDone = 0
        # print(f'Mav{self.ID}等待其他飞机同步发送起飞位置{pos}')
        REG.barrier.wait()
        curPos=self.mav.uavPosNED
    ...
    def QuadPos(self,pos):  # ID = 4
        self.isDone = 0
        # print(f'Mav{self.ID}等待其他飞机同步发送{pos}')
        REG.barrier.wait()
        self.mav.SendPosNED(pos[0],pos[1],pos[2])
    ...
    def QuadVel(self,vel):  # ID = 5
        self.isDone = 0
        # print(f'Mav{self.ID}等待其他飞机同步发送{vel}')
        REG.barrier.wait()
    ...
    def hover(self):  # ID = 6
        self.isDone = 0
        # print(f'Mav{self.ID}等待其他飞机同步悬停')
        REG.barrier.wait()
        curPos=self.mav.uavPosNED
    ...
    def MissionRun(self, setpoint):
        ...
        for setp in setpoint:
            REG.barrier.wait()
            self.mav.SendVelNED(setp[0],setp[1],setp[2],setp[3])
            time.sleep(0.01)
        
        self.mav.SendVelNED(0,0,0,0)
        print(f"执行时长：  {time.time() - start_T}")
    ...
    def FaultInject(self,param):  # ID = 9
        ...
        # print(f'Mav{self.ID}等待其他飞机同步故障注入')
        REG.barrier.wait()
